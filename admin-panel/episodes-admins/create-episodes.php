...
<?php
    ...
    if(empty($_POST['name']) OR empty($_POST['show_id'])){
          ...
          $dir_thumbnail = "images/" . basename($thumbnail);
          $dir_video = "videos/" . basename($video);

          $insert = $conn->prepare("INSERT INTO episodes(episode_name, show_id, thumbnail, video) VALUE(:name, :show_id, :thumbnail, :video)");
          ...
          if(move_uploaded_file($_FILES['thumbnail']['tmp_name'], $dir_thumbnail)){
            header("location: show-episodes.php");
          }
